...
resource "aws_db_subnet_group" "wordpress-rds" {
  ...
  subnet_ids = ["${ aws_subnet.db-a.id }", "${ aws_subnet.db-b.id }", "${ aws_subnet.db-c.id }"]
  tags {
...
resource "aws_db_instance" "wordpress" {  
  ...
  instance_class = "db.t2.micro" # note this instance class does not support encryption at rest (would normally encrypt RDS 
  multi_az = false
  name = "wordpress"
